<template>
  <div class="min-h-screen bg-base-900 flex items-center justify-center p-4">
    ...
    <div class="fixed inset-0 overflow-hidden pointer-events-none">
      <div class="absolute top-[-20%] left-[30%] w-[600px] h-[600px] bg-violet-600/10 rounded-full blur-3xl" />
    </div>

    <div class="glass rounded-2xl w-full max-w-md p-8 relative z-10">

      <div class="mb-8">
        <h1 class="text-3xl font-bold gradient-text mb-1">Welcome back</h1>
        <p class="text-base-400 text-sm">Sign in to continue to Notely</p>
      </div>

      <div class="space-y-4">
        <div>
          <label class="block text-xs font-medium text-base-400 uppercase tracking-wider mb-1.5">
            Email
          </label>
          <input
            v-model="email"
            ...
            class="w-full bg-base-800 border border-base-500 rounded-xl px-4 py-3 text-sm text-white placeholder-base-400 focus:outline-none focus:border-violet-500 focus:glow transition"
          />
        </div>
      ...
      <p class="text-center text-sm text-base-400 mt-6">
        ...
        <RouterLink to="/register" class="text-violet-400 hover:text-violet-300 font-medium transition">
          Register
        </RouterLink>
      </p>
